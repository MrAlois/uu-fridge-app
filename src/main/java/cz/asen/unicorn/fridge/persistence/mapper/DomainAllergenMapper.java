...
public class DomainAllergenMapper {
     ...
     */
    public static @NotNull Set<Allergen> parseAllergens(String allergens){
        ...
        return Arrays.stream(allergens.split(DIETARY_RESTRICTION_DELIMITER))
                ...
                .map(allergen -> {
                    try {
                        return Allergen.valueOf(allergen);
     ...
     */
    @Contract("_ -> new")
    public static @NotNull String parseAllergens(Set<Allergen> allergens){
        if(allergens == null) return "";
        return String.join(DIETARY_RESTRICTION_DELIMITER, allergens.stream().map(Allergen::name).toList());
    }
}
